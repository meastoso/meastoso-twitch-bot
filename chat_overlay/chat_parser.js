...
function totalMessagesCheck(user) {
	...
	var ret = false;
	if (parseInt(num) < parseInt(chatOverlayConfig.chatapp.filters.user.totalMessagesThreshold)) {
...
function totalBitsDonatedCheck(user) {
	...
	if (user.badges !== undefined && user.badges !== null && user.badges.bits !== undefined && user.badges.bits !== null && user.badges.bits != '') {
		...
		if (num >= parseInt(chatOverlayConfig.chatapp.filters.user.totalBitsDonatedThreshold)) {
