...
function getConfig() {
	...
	if (configJSON == null) {
		try {
			...
			configJSON = require("../config.json"); // first time load from file
		}
		catch (e) {
	...
	return configJSON; // if not null just return cache object
}
...
function getInitialConfigJSON() {
	 ...
	 */
	config.chatapp.overlaywindow.transparent = "false";
	/**
